...
@router.get("", response_model=list[SubscriptionPlanRead], dependencies=[Depends(require_roles("accountant","admin"))])
...
@router.get("/{plan_id}", response_model=SubscriptionPlanRead, dependencies=[Depends(require_roles("accountant","admin"))])
...
@router.post("", response_model=SubscriptionPlanRead, status_code=status.HTTP_201_CREATED, dependencies=[Depends(require_roles("accountant"))])
...
@router.patch("/{plan_id}", response_model=SubscriptionPlanRead, dependencies=[Depends(require_roles("accountant"))])
